...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        # Get all zkLogin users (users with accounts that have algorand_address set)
        ...
        with transaction.atomic():
            for user in zklogin_users:
                ...
                algorand_address = user_account.algorand_address if user_account else 'N/A'
                self.stdout.write(f'\nProcessing user: {user.username} (algorand_address: {algorand_address[:20] if algorand_address != "N/A" else "N/A"}...)')
                user_earned = 0
                user_claimed = 0
                user_confio = 0
