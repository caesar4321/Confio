...
def setup_assets():
    ...
    print("\n" + "="*60)
    print("cUSD CONTRACT - ASSET SETUP")
    print("="*60)
    ...
    params.fee = max(getattr(params, "min_fee", 1000) * 3, 3000)
    
    print("\nCalling setup_assets...")
    
    # Fund app min balance and fees (0.6 ALGO)
    ...
    atc.add_transaction(TransactionWithSigner(fund, signer))
    ...
    try:
        ...
        deployment["deployment_status"] = "Assets configured. Ready for use!"
        ...
        print("NEXT STEPS:")
        print("="*60)
        print("\n1. REKEY RESERVE (optional but recommended):")
        print(f"   Rekey the cUSD reserve account to the app address")
        print(f"   App Address: {deployment['app_address']}")
        print("\n2. TEST ADMIN MINTING:")
        print(f"   python test_cusd_minting.py")
