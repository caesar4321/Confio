...
⚠️ IMPORTANT: This script DEFINES the actual cUSD token parameters!
The token is created here with AssetConfigTxn, NOT in cusd.py.
The cusd.py smart contract only controls minting/burning behavior.

Token Creation (this file):
- Creates ASA with maximum supply (2^64-1) in reserve
- Sets clawback to allow contract-controlled minting

Smart Contract (cusd.py):
- Controls WHEN tokens can be minted (collateral requirements)
- Manages HOW tokens are minted (from reserve via clawback)
- Does NOT define token parameters (supply, decimals, etc.)
"""
...
def create_cusd_asset(algod_client, creator_private_key, creator_address):
    ...
    # ⚠️ THESE ARE THE ACTUAL cUSD TOKEN PARAMETERS - NOT DEFINED IN cusd.py!
    # The smart contract (cusd.py) will control minting/burning of this token,
    # but the token itself is created HERE with these parameters.
    # Create cUSD asset with maximum possible supply (2^64 - 1)
    # This allows unlimited minting based on collateral
    txn = AssetConfigTxn(
        sender=creator_address,
        sp=params,
        total=2**64 - 1,  # Maximum supply - HARDCODED HERE, NOT IN CONTRACT!
