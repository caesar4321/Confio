  ...
  const handlePay = async (item: any) => {
    ...
    try {
      ...
      const submitRes = await submitPayrollItem({
        variables: { payrollItemId: item.itemId, signedTransaction: signedB64 },
      });
      ...
      <View style={styles.balanceCard}>
        <View>
          ...
          <Text style={styles.balanceValue}>
            {vaultLoading ? '...' : `${vaultData?.payrollVaultBalance?.toFixed?.(2) || '0.00'} cUSD`}
          </Text>
          <Text style={styles.balanceHint}>Agrega fondos si ves rechazos por saldo insuficiente.</Text>
        </View>
        <TouchableOpacity style={styles.refreshButton} onPress={() => { refetch(); refetchVault(); }}>
          <Icon name="refresh-ccw" size={16} color={colors.text} />
        </TouchableOpacity>
      </View>
