...
import { PREPARE_ZKLOGIN, INITIALIZE_ZKLOGIN, FINALIZE_ZKLOGIN } from '../apollo/queries';
...
export class AuthService {
  ...
  public async signInWithApple() {
    ...
    try {
      ...
      // STEP 8: Initialize zkLogin (to get auth tokens)
      ...
      const { authAccessToken, authRefreshToken } = initData.initializeZkLogin;
      ...
      // STEP 9: Now regenerate salt and keypair with real Apple sub
      console.log('Step 9: Regenerating salt and keypair with real Apple sub...');
      ...
      // STEP 10: Get the extended ephemeral public key
      const extendedEphemeralPublicKey = ephemeralKeypair.getPublicKey().toBase64();

      // STEP 11: Finalize zkLogin (JWT already has correct nonce!)
      console.log('Step 11: Finalizing zkLogin...');
      const { data: finalizeData } = await apolloClient.mutate({
        mutation: FINALIZE_ZKLOGIN,
        variables: {
          input: {
            extendedEphemeralPublicKey,
            jwt: appleAuthResponse.identityToken,
            ...
            accountIndex: accountContext.index,
            deviceFingerprint: deviceFingerprint ? JSON.stringify(deviceFingerprint) : null
            // NO computedNonce needed - JWT already has the correct nonce!
          }
        }
      });
  ...
  private deriveEphemeralKeypair(saltB64: string, sub: string, clientId: string): Ed25519Keypair {
    try {
      ...
      // Ed25519 requires a 32-byte seed, but our salt is now 16 bytes for Mysten compatibility
      // We need to expand the 16-byte salt to 32 bytes deterministically
      let seed: Uint8Array;
      if (saltBytes.length === 16) {
        // For 16-byte salt (Mysten compatible), double it to create 32-byte seed
        seed = new Uint8Array(32);
        seed.set(saltBytes, 0);  // First 16 bytes
        seed.set(saltBytes, 16); // Repeat for last 16 bytes
      } else if (saltBytes.length >= 32) {
        // For backward compatibility with 32-byte salts
        seed = saltBytes.slice(0, 32);
      } else {
        throw new Error(`Invalid salt length: ${saltBytes.length} bytes (expected 16 or 32)`);
      }
