...
class AlgorandService {
  ...
  private async ensureInitialized() {
    if (!this.algosdk) {
      try {
        // Lazy load the libraries (polyfills are already loaded in index.js)        const algosdk = await import('algosdk');
        // Web3Auth is no longer used - we use secure deterministic wallet instead
        // Skip Web3Auth imports to avoid casting errors

        // Debug algosdk import        this.algosdk = algosdk;
  ...
  */
  ...
  async clearWallet() {
    try {
      ...
      try {
        await secureDeterministicWallet.clearWallet();      } catch (error: any) {
  ...
  private async ensureWalletRestored(): Promise<boolean> {
    try {
      ...
      this.currentAccount = { addr: wallet.address, sk: null };      return true;
  ...
  async signAndSubmitSponsoredTransaction(
    ...
    try {
      ...
      const sponsorTxnBytes = typeof sponsorTransaction === 'string'
        ...
        : sponsorTransaction;      // signTransactionBytes will ALSO attempt to heal, but doing it upfront is safer for the group logic
  ...
  async processSponsoredOptIn(
    ...
    try {
      ...
      if (!this.currentAccount) {
        ...
        // If still null, we iterate the logic again (or just rely on signTransactionBytes later)        return false;
      }
      ...
      // Single-asset fallback is no longer supported (backend sends arrays)      return false;
