...
export const PayrollRecipientModal: React.FC<Props> = ({ visible, onClose, onChanged }) => {
  ...
  const handleAdd = async () => {
    try {
      ...
      if (mode === 'phone') {
        if (!phone.trim()) {
          Alert.alert('Ingresa teléfono', 'Por favor ingresa el número de teléfono del destinatario');
          return;
        }
        ...
        if (!username.trim()) {
          Alert.alert('Ingresa usuario', 'Por favor ingresa el @usuario de Confío');
          return;
        }
      ...
      if (!match || !match.isOnConfio) {
        Alert.alert('No encontrado', 'No encontramos un usuario de Confío con ese dato.');
        return;
      }
      if (!match.activeAccountId) {
        Alert.alert('Cuenta faltante', 'El usuario no tiene una cuenta activa seleccionable.');
        return;
      }
      ...
      if (resCreate.data?.createPayrollRecipient?.success) {
        ...
        Alert.alert('Error', resCreate.data?.createPayrollRecipient?.errors?.[0] || 'No se pudo guardar');
      }
    } catch (e) {
