...
# Deploy script for zkLogin prover V2
# Usage: ./deploy-v2.sh <EC2_IP> <SSH_KEY_PATH>

EC2_IP=${1:-16.62.221.198}
SSH_KEY=${2:-~/.ssh/id_rsa}

echo "🚀 Deploying zkLogin prover V2 to EC2 instance: $EC2_IP"
...
scp -i "$SSH_KEY" -o StrictHostKeyChecking=no \
    zklogin-complete-prover-v2.js \
    zklogin-input-parser-v2.js \
...
ssh -i "$SSH_KEY" -o StrictHostKeyChecking=no ec2-user@$EC2_IP << 'EOF'
    ...
    mv zklogin-complete-prover.js zklogin-complete-prover-old.js 2>/dev/null || true
    mv zklogin-input-parser.js zklogin-input-parser-old.js 2>/dev/null || true
    
    # Rename new versions
    echo "Installing new versions..."
    mv zklogin-complete-prover-v2.js zklogin-complete-prover.js
    mv zklogin-input-parser-v2.js zklogin-input-parser.js
