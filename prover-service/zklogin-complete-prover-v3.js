...
async function generateRealProof(inputs) {
    try {
        ...
        console.log('  - ext_nonce_length:', inputs.ext_nonce_length);
        
        // Write inputs to file for debugging
        fs.writeFileSync('/tmp/zklogin-inputs-v3.json', JSON.stringify(inputs, null, 2));
        console.log('📝 Inputs written to /tmp/zklogin-inputs-v3.json');
...
app.post('/v1', async (req, res) => {
    ...
    try {
        const { 
            ...
            keyClaimName,
            audience,
            originalNonce  // NEW: Accept the original 27-char nonce
        } = req.body;
        ...
        console.log(`📏 JWT nonce length: ${jwtNonce?.length} chars`);
        
        // Check if we have the original nonce for Apple Sign-In
        if (jwtNonce?.length === 44 && !originalNonce) {
            console.error('❌ Apple Sign-In detected but original nonce not provided!');
            console.log('The JWT contains a hashed 44-char nonce but we need the original 27-char nonce');
            throw new Error('Original 27-char nonce required for Apple Sign-In. Please pass the nonce that was used to initiate OAuth.');
        }
        
        if (originalNonce) {
            console.log(`✅ Using provided original nonce: ${originalNonce.length} chars`);
        }
        ...
        const inputs = prepareZkLoginInputs(
            ...
            salt,
            keyClaimName || 'sub',
            originalNonce  // Pass the original nonce
        );
...
app.listen(port, '0.0.0.0', () => {
    console.log(`
🚀 Complete zkLogin Prover V3 running on port ${port}
📁 Circuit files:
   - WASM: ${fs.existsSync(CIRCUIT_WASM_PATH) ? '✅' : '❌'} ${CIRCUIT_WASM_PATH}
   - zkey: ${fs.existsSync(CIRCUIT_ZKEY_PATH) ? '✅' : '❌'} ${CIRCUIT_ZKEY_PATH}
🔑 JWKS Support:
   - Apple: ✅ (requires original nonce)
   - Google: ✅
🔗 Endpoints:
   - POST /v1 - Generate zkLogin proof
   - GET /health - Check prover status
📝 Critical Updates:
   - FIXED: Now accepts original 27-char nonce for Apple Sign-In
   - The circuit needs the original nonce, not the hashed one from JWT
   - Client must pass the nonce that was used to initiate OAuth
    `);
});