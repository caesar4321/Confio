...
/**
 * Complete zkLogin input parser for circuit
 * Converts JWT and params into circuit-compatible format
 */
...
// Removed - handled inline with proper 128-bit split
...
export function prepareZkLoginInputs(
    ...
    const paddedUnsignedJwt = stringToAsciiArray(unsignedJwt, 1600);  // Circuit expects 1600 (from example)
    
    // Find field positions in base64 payload
    // Look for the base64 encoding of the field names without quotes
    ...
    // Find value positions (absolute positions in payload)
    const audValueIndex = findSubstringIndex(payloadStr, aud);
    const nonceValueIndex = findSubstringIndex(payloadStr, nonce);
    const kcValueIndex = findSubstringIndex(payloadStr, payloadJson[keyClaimName] || sub);
    ...
    for (let i = 0; i < 32; i++) {
        ...
        if (chunk.length > 0) {
            // Convert chunk to BigInt (little-endian)
            let value = BigInt(0);
    ...
    // Don't JSON.stringify aud - it should be the raw value
    // Use exact sizes from the circuit's input.json example
    const extAud = stringToAsciiArray(aud, 160);  // Example has 160
    const extNonce = stringToAsciiArray(nonce, 44);  // Example has 44
    const extKc = stringToAsciiArray(payloadJson[keyClaimName] || sub, 126);  // Example has 126
    
    // Email verified field (if exists)
    const evValue = payloadJson.email_verified ? 'true' : 'false';
    const evIndex = payloadJson.email_verified ? findBase64Pattern(payloadB64, 'email_verified') : 0;
    const evColonIndex = payloadJson.email_verified ? findSubstringIndex(payloadStr, '"email_verified":') : 0;
    const evValueIndex = payloadJson.email_verified ? findSubstringIndex(payloadStr, evValue) : 0;
    const extEv = stringToAsciiArray(evValue, 53);  // Example has 53
    ...
    const inputs = {
        ...
        aud_value_index: (audValueIndex - audColonIndex).toString(),
        ...
        ev_length_b64: '20',
        ev_name_length: '14', // "email_verified"
        ev_value_index: (evValueIndex - evColonIndex).toString(),
        ...
        ext_nonce: extNonce,
        ext_nonce_length: nonce.length.toString(),
        ...
        kc_value_index: (kcValueIndex - kcColonIndex).toString(),
        ...
        nonce_length_b64: Math.ceil((7 + nonce.length) * 4 / 3).toString(), // Base64 length of "nonce":"value"
        nonce_value_index: (nonceValueIndex - nonceColonIndex).toString(),
...
// Test code removed for ES module compatibility