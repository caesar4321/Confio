...
class LoginAttestationTests(TestCase):
    ...
    @override_settings(APP_CHECK_LOGIN_ENFORCE=True)
    ...
    @override_settings(APP_CHECK_LOGIN_ENFORCE=False)
    ...
    @override_settings(APP_CHECK_LOGIN_ENFORCE=False)
    def test_warning_mode_does_not_disable_sensitive_operations(self):
        result = app_check_service.verify_and_record(user=None, token='', action='payment')
        self.assertFalse(result['success'])
