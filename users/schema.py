...
class Query(EmployeeQueries, graphene.ObjectType):
	...
	def resolve_account_balance(self, info, token_type):
		...
		try:
			...
			print(f"AccountBalance resolver - Found account with Aptos address: {account.aptos_address}")
		...
		# Check if account has a Aptos address
		if not account.aptos_address:
			print(f"AccountBalance resolver - Account has no Aptos address, returning 0")
			return "0"
...
class UpdateAccountAptosAddress(graphene.Mutation):
	...
	def mutate(cls, root, info, aptos_address):
		...
		try:
			...
			logger.error(f"Error updating account Aptos address: {str(e)}")
	...
	def mutate(cls, root, info, phone_numbers):
		...
		for phone in phone_numbers:
			...
			try:
				...
				# Generate a valid Aptos address using hash of phone number
				# Aptos addresses are 0x + 64 hex characters (32 bytes)
				...
				personal_account = Account.objects.create(
					...
					account_index=0,
					aptos_address=test_aptos_address  # Valid Aptos address format
				)
