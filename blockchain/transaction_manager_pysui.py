...
class TransactionManagerPySui:
        ...
        try:
            ...
            async with await get_pysui_client() as client:
                if token_type == 'CUSD':
                    ...
                    balances = await client.get_balance(account.sui_address, coin_type)
                    return balances.get(coin_type, Decimal('0'))
                    
        except Exception as e:
            logger.error(f"Error getting {token_type} balance for {account.sui_address}: {e}")
            return Decimal('0')
    ...
    async def get_all_balances(cls, account: Account) -> Dict[str, Decimal]:
        ...
        try:
            ...
            async with await get_pysui_client() as client:
                ...
                all_balances = await client.get_balance(account.sui_address)
            ...
            logger.error(f"Error getting all balances for {account.sui_address}: {e}")
    ...
    async def invalidate_balance_cache(cls, account: Account, token_type: str):
        ...
        cache_key = f"balance:{account.sui_address}:{cls.TOKEN_CONFIGS[token_type]['type']}"
        cache.delete(cache_key)
        
        # Invalidate all balances cache
        cache_key_all = f"balance:{account.sui_address}:all"
        cache.delete(cache_key_all)
        
        logger.info(f"Invalidated balance cache for {account.sui_address} {token_type}")
    ...
    async def sync_balance_to_database(cls, account: Account):
        ...
        try:
            ...
            logger.info(f"Synced balances to database for {account.sui_address}")
