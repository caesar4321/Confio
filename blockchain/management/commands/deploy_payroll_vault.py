...
Constructor: (cusdPlusVault, feeRecipient, owner)
  - cusdPlusVault  = CUSD_PLUS_VAULT_ADDRESS (live 0x3C29…3Ed1)
  - feeRecipient   = BSC_FEE_RECIPIENT_ADDRESS (the dedicated payments
                     treasury — payroll fee shares land in the same place)
  - owner          = the 3-of-5 Safe (can only rotate feeRecipient and
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        fee_recipient = getattr(settings, "BSC_FEE_RECIPIENT_ADDRESS", "") or ""
        if not cusd_plus:
            raise CommandError("CUSD_PLUS_VAULT_ADDRESS not configured")
        if not fee_recipient:
            raise CommandError(
                "BSC_FEE_RECIPIENT_ADDRESS not configured — create and "
                "custody-verify the fee treasury first (never the sponsor)")
        ...
        self.stdout.write(f"feeRecipient  = {fee_recipient}")
        ...
        self.stdout.write(f"  feeRecipient = {call_addr('feeRecipient()')}")
