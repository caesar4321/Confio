...
Constructor: (cusdPlus, cusd, usdt, confio, signer, owner)
  - cusdPlus  = CUSD_PLUS_VAULT_ADDRESS (live 0x3C29…3Ed1; vault shares
                are one of the two CHARGE denominations)
  - cusd      = CUSD_VAULT_ADDRESS (universal non-yield payment dollars)
  - usdt      = BSC USDT 0x55d3…7955 — retained only so already-signed
                legacy payment batches can settle during migration; new
                authorizations never use raw USDT after fee activation
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        tokens = {"cusdPlus": cusd_plus, "cusd": cusd, "usdt": USDT_BSC, "confio": confio}
        ...
        if ctor_arity != 6:
            raise CommandError(
                f"artifact constructor takes {ctor_arity} args, expected 6 "
                "(cusdPlus, cusd, usdt, confio, signer, owner) — rebuild with `forge build`")
        for getter in ("CUSD_PLUS", "CUSD", "USDT", "CONFIO", "paymentSigner", "owner"):
        ...
        ctor_args = abi_encode(
            ...
            [cusd_plus, cusd, USDT_BSC, confio, deployer, SAFE],
        )
