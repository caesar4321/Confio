...
class Command(BaseCommand):
        ...
        try:
            ...
            if dry_run:
                ...
                for account in accounts:
                    self.stdout.write(f"   - {account.user.email}: {account.algorand_address[:16]}...")
                return
    ...
    async def distribute_token(self, account, token_type, amount, treasury_key):
        ...
        try:
            ...
            self.stdout.write(f"      📤 Sending {amount} {token_type} to {account.algorand_address[:16]}...")
            ...
            logger.exception(f"Failed to distribute {token_type} to {account.algorand_address}")
            return {
                'success': False,
                'error': str(e)
            }