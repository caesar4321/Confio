...
import { fromB64, toB64, bcs } from '@mysten/bcs';
...
// Interface for zkLogin signature request
interface ZkLoginSignatureRequest {
  ephemeralSignature: string;      // Base64 ephemeral signature
  ephemeralPublicKey: string;      // Base64 ephemeral public key
  zkProof: {                       // zkProof with a, b, c points
    a: string[];
    b: string[][];
    c: string[];
  };
  maxEpoch: string;               // Maximum epoch as string
  subject: string;                // JWT subject
  audience: string;               // JWT audience (e.g., 'apple')
  userSalt: string;              // User salt for address derivation
  issuer?: string;               // JWT issuer (optional, will be derived from audience)
}

// Map audience to issuer
const getIssuerFromAudience = (audience: string): string => {
...
function genAddressSeed(saltBase64: string, name: string, value: string, audience: string): string {
...
app.post('/bcs-signature', async (req, res) => {
  try {
    const {
      ...
      userSalt,
      issuer
    }: ZkLoginSignatureRequest = req.body;
    ...
    try {
      ...
      try {
        ...
        const zkLoginInputs = bcs.struct('ZkLoginInputs', {
          ...
          addressSeed: bcs.u256()  // Changed from string to u256
        });
        
        const zkLoginSig = bcs.struct('ZkLoginSignature', {
          inputs: zkLoginInputs,
          maxEpoch: bcs.u64(),
          userSignature: bcs.vector(bcs.u8()) // Full Ed25519 signature with flag + sig + pubkey
        });
        ...
        const zkLoginSignature = toB64(fullSignature);
