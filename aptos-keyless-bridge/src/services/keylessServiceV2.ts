...
interface SimpleFeePayerRequest {
  ...
  amount: number;
  tokenType: string;
  senderAuthenticator: string; // Base64 encoded authenticator from signing message
  senderAuthenticatorBcs?: string; // A/B Test: Base64 encoded authenticator from raw BCS
  // EXPERIMENTAL: Add keyless account data for direct signing
  jwt?: string;
  ephemeralKeyPair?: {
    privateKey: string;
    publicKey: string;
...
export class KeylessServiceV2 {
   ...
   */
  async submitCachedTransaction(transactionId: string, senderAuthenticatorBase64: string, senderAuthenticatorBcsBase64?: string) {
    try {
      if (!this.pendingTransactions) {
      ...
      try {
        ...
        if (senderAuthenticatorBcsBase64) {
          ...
          try {
            ...
            const bcsDeserializer = new Deserializer(new Uint8Array(bcsAuthBytes));
            const senderAuthenticatorBcs = AccountAuthenticator.deserialize(bcsDeserializer);
